...
def postprocess_agent_loop_outputs(inputs: list[AgentLoopOutput], tokenizer, config) -> DataProto:
    ...
    for input in inputs:
        ...
        response_logprobs = torch.tensor(input.response_logprobs + [0.0] * pad_size).unsqueeze(0)  # [1, response_length]
        response_logprobs_list.append(response_logprobs)
    rollout_log_probs = torch.cat(response_logprobs_list, dim=0)  # [bsz, response_length]
...
def prepare_single_generation_data(batch_dict, global_steps, rollout_n) -> DataProto:
    ...
    # Setting agent - partial_tool_agent, that supports partial
    full_batch.non_tensor_batch["agent_name"] = np.array(["partial_tool_agent"] * len(full_batch), dtype=object)
